...
type Props = { spec: StrategyCaseSpec; onExit: () => void; children: ReactNode }

export default function StrategyCaseShell({ spec, onExit, children }: Props) {
  return (
    <section className='strategy-runner-page'>
      <header className='strategy-shell-head'>
        <button type='button' className='strategy-back' onClick={onExit}><ArrowLeft aria-hidden='true' />返回案例地图</button>
