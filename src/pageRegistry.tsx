...
export const pageLoaders: Record<Page, Loader> = {
  ...
  progress: async () => { const [module] = await Promise.all([import('./components/foundation/ProgressPage'), import('./learning-os.css')]); return { default: ({ go }) => <module.default go={go} /> } },
  ...
  llm: async () => { const [{ default: Notice }, { default: Track }, { llmChapters }] = await Promise.all([import('./components/strategy/LegacyStrategyNotice'), import('./components/CourseTrack'), import('./courseData'), import('./lab.css')]); return { default: ({ go }) => <><Notice go={go} /><Track chapters={llmChapters} tone='llm' onOpenLab={() => go('lab')} /></> } },
  image: async () => { const [{ default: Notice }, { default: Track }, { imageChapters }] = await Promise.all([import('./components/strategy/LegacyStrategyNotice'), import('./components/CourseTrack'), import('./courseData'), import('./lab.css')]); return { default: ({ go }) => <><Notice go={go} /><Track chapters={imageChapters} tone='image' onOpenLab={() => go('lab')} /></> } },
