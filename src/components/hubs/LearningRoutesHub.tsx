import { ArrowRight, BookOpen, BrainCircuit, Compass, FlaskConical, GitBranch, Image, Network, Sigma, Sparkles } from 'lucide-react'
  ...
  return (
    <section className='lo-hub-page lo-routes-page'>
      <header className='lo-routes-intro'>
        ...
        <p>六条主线路线覆盖算法基础、生成模型与 Agent 工程；另有两条前沿探索路线，用于关注仍在早期的方向。每条路线都连接课程、实验和评审，不需要在二十多个页面里猜入口。</p>
          ...
          <div>
            {mainline.map(({ icon: Icon, ...route }, index) => (
              <article key={route.title} className='lo-route-chapter'>
                ...
                <footer><button type='button' onClick={() => go(route.page)}>进入路线<ArrowRight /></button><button type='button' className='is-quiet' onClick={() => go(route.labPage, { experiment: route.experiment })}>{route.lab}</button></footer>
              </article>
            ))}
          </div>
        </section>
