...
const foundationCase = {
  id: 'foundation-feedback-loop',
  routeId: 'foundation',
  routeLabel: '算法基础',
  title: '上线策略如何改变模型学到什么？',
  question: '流量、触达与复核策略，会留下怎样的训练反馈？',
  duration: '预计 3–5 分钟',
  page: 'foundation-lab',
  options: { experiment: 'case-overconfident' },
  spec: undefined,
} as const satisfies CaseCatalogItem

function fromSpec<const TSpec extends StrategyCaseSpec>(spec: TSpec, extra: { frontier?: boolean } = {}) {
  return {
    id: spec.id,
    ...
    page: 'strategy-case' as const,
    options: { case: spec.id },
    spec,
    ...extra,
  }
}
...
export const strategyCaseCatalog = [
  ...
  fromSpec(imageSpec),
  fromSpec(agentSpec),
  fromSpec(agentBookSpec),
  fromSpec(distillSpec),
  fromSpec(evaluatorTrustSpec, { frontier: true }),
  fromSpec(worldModelSpec, { frontier: true }),
] as const
...
export const flagshipCaseIds = new Set<CaseId>(['rag-budget', 'refund-gate', 'new-information'])
export const frontierCaseIds = new Set<CaseId>(
  strategyCaseCatalog.filter((item) => 'frontier' in item && item.frontier).map((item) => item.id),
)
export const frontierRouteIds = new Set<RouteId>(
  strategyCaseCatalog.filter((item) => 'frontier' in item && item.frontier).map((item) => item.routeId),
)
