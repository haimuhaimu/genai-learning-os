...
系统 MUST 保留现有 34 个 canonical page、`map` 别名以及 `module`、`experiment`、`node`、`section`、`chapter`、`card`、`case`、`paper` 深链参数的可用性；系统 MUST NOT 删除、重命名或重新解释已有内容 ID、进度 ID、存储键及其已保存值。

#### Scenario: 旧顶层直达 URL 继续可用
- **WHEN** 用户打开 `page=videos`、`page=papers`、`page=reviews`、`page=progress`、`page=handbook` 或 `page=co-build` 等不再位于主导航的旧 URL
...
- **AND** 不把该参数替换为新的 registry ID

#### Scenario: 已有进度无迁移损失
- **WHEN** 浏览器中存在变更前写入的学习进度
- **THEN** 首页、聚合页和详情页读取相同进度 ID 与存储值
- **AND** 用户无需清空或手工迁移数据即可继续学习
