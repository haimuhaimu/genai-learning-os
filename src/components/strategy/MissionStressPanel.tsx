...
import { readStrategyEvidence, saveStrategyEvidence } from '../../strategyEvidence'
import type { CaseId } from './caseCatalog'
import { evaluateMission, runStressPreset } from './missionEngine'
import type { ControlValues, MissionStressRecord, StrategyCaseSpec } from './types'

export default function MissionStressPanel({ caseId, spec, controls, result, onResult }: { caseId: CaseId; spec: StrategyCaseSpec; controls: ControlValues; result?: MissionStressRecord; onResult: (result: MissionStressRecord) => void }) {
  ...
  const run = (presetId: string) => {
    ...
    const record = { presetId, passed: next.evaluation.passed, ranAt: now, metrics: next.evidence.metrics.map(({ id, label, display, value }) => ({ id, label, display, value })) }
    saveMissionStress(caseId, record)
    if (next.evaluation.passed) {
      const previous = readStrategyEvidence().find((item) => item.caseId === caseId)
      saveStrategyEvidence(previous ? { ...previous, missionEvidence: { passedStressPresetIds: [presetId], lastStressAt: now }, updatedAt: now } : { caseId, routeId: spec.routeId, level: 1, controls, metrics: next.evidence.metrics, summaryText: '', updatedAt: now, missionEvidence: { passedStressPresetIds: [presetId], lastStressAt: now } })
    }
