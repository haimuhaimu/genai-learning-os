...
优先复用 [`examples/strategy-case/exampleCase.test.mjs`](../examples/strategy-case/exampleCase.test.mjs) 中的三种关系测试：

1. **收益与代价同时变化**：成对比较基线与新策略，确认改善目标时也付出了预期代价。
2. **阈值触发停手**：比较阈值前后的输入，断言学习者能从下一步动作和风险提示看见 veto。
3. **不保证单调改善**：允许中间指标继续上升，同时证明最终目标可能因边际收益下降而变差。

常见反例是只保存整段快照、只断言收益不看成本、用生产函数本身计算期望值，或检查源码文本而不观察公开输出。这些测试即使通过，也没有证明案例宣称的教学关系。
