...
import { getStrategyCase } from './caseRegistry'
...
import type { ControlValue, ControlValues } from './types'
...
export default function StrategyCaseRunner({ caseId, go }: Props) {
  ...
  if (!spec) return <section className='strategy-not-found'><span>策略案例</span><h1>没有找到这个案例</h1><p>案例 ID 可能已失效，请从策略案例地图重新进入。</p><button type='button' onClick={() => go('strategy-cases')}>返回案例地图</button></section>
