...
Dependabot 每周一 09:00（Asia/Shanghai）检查根目录的 npm/pnpm 依赖，同时最多保留 3 个版本更新 PR。处理这些 PR 时：

1. 每个 PR 只处理依赖升级，不混入功能、内容或重构。
2. 核对 `package.json` 与 `pnpm-lock.yaml` 的变化，确认没有意外新增包、安装脚本或来源变更。
3. 等待现有 `pnpm ci:check` 全部通过；失败时先定位兼容性问题，不跳过门禁，也不自动合并。
4. major 升级必须额外阅读发布说明与迁移指南，记录行为变化并完成针对性回归验证。
