    ...
    <section className='strategy-controls-panel' aria-labelledby='strategy-controls-title'>
      ...
      <div className='strategy-control-grid'>
        {schema.map((control) => {
          ...
          if (control.type === 'toggle') return <label className='strategy-toggle' key={control.id} htmlFor={`strategy-${control.id}`}><span><b>{control.label}</b>{control.detail ? <small>{control.detail}</small> : null}</span><input id={`strategy-${control.id}`} type='checkbox' checked={Boolean(value)} onChange={(event) => onChange(control.id, event.target.checked)} /><i aria-hidden='true' /></label>
          ...
          return <fieldset id={`strategy-${control.id}`} tabIndex={-1} className='strategy-choice' key={control.id}><legend>{control.label}</legend><div>{control.options.map((option) => <button type='button' key={String(option.value)} className={value === option.value ? 'is-active' : ''} aria-pressed={value === option.value} onClick={() => onChange(control.id, option.value)}>{option.label}</button>)}</div>{control.detail ? <small>{control.detail}</small> : null}</fieldset>
        })}
      </div>
    </section>
  )
